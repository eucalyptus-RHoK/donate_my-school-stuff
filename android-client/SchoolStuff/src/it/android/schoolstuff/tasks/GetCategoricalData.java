...
public class GetCategoricalData extends AsyncTask<Boolean, Integer, Object> {
	...
	public static List<Integer> catID;
	public static List<Integer> schID;
		...
		try {
			...
			try {
				...
				for (int i = 0; i < jcat.length(); i++) {
					...
					catID.add(jcat.getJSONObject(i).getInt("pk"));
				}

				schools = new LinkedList<String>();
				schID = new LinkedList<Integer>();
				...
				for (int i = 0; i < jschool.length(); i++) {
					...
					schID.add(jschool.getJSONObject(i).getInt("pk"));
				}
